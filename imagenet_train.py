...
from losses import LDAMLoss, BalancedSoftmaxLoss, EffBalancedSoftmaxLoss
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.loss_type == 'CE':
            ...
            criterion = EffBalancedSoftmaxLoss(cls_num_list=cls_num_list_cuda).cuda(args.gpu)
        ...
        # train for one epoch
        train(train_loader, model, criterion, optimizer1, optimizer2,   epoch, args, weighted_train_loader=weighted_train_loader, median_train_loader = median_train_loader)
...
def train(train_loader, model, criterion, optimizer1, optimizer2, epoch, args, log=None,
              tf_writer=None, weighted_train_loader=None, median_train_loader=None):
    ...
    if args.data_aug == 'CMeO':
        ...
        criterion2 = EffBalancedSoftmaxLoss(cls_num_list=cls_num_list_cuda).cuda(args.gpu)
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if args.data_aug == 'CMeO' and args.start_data_aug < epoch < (
            ...
            loss1 = criterion(output1, target) * lam1 + criterion(output1, target_m) * (1. - lam1)
            loss2 = criterion2(output2, target_a) * lam2 + criterion2(output2, target_b) * (1. - lam2)
